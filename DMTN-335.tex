...
\citep[LSST;][]{Ivezic2019} will produce the deepest wide-field optical
imaging survey to date. Transforming this imaging into reliable galaxy
catalogues requires robust source detection and deblending, currently
performed by the \scarletlite algorithm \citep{Melchior2018} within the
LSST Science Pipelines. In this tech note we present a quantitative
assessment of the current pipeline using controlled source injection
experiments on real LSSTCam imaging (data release
\texttt{w\_2025\_37/DM-52496}). We inject mock observations of NewHorizon
simulated galaxies \citep{Martin2022} into calibrated single-exposure
images, process the modified exposures through the full pipeline to produce
deep coadds, and compare the resulting catalogues against those derived from
the original, unmodified coadds. The injected sources span $r$-band mean
surface brightnesses (measured within \Reff) of $18 < \mur < 30$
$\mathrm{mag\,arcsec^{-2}}$, effective radii of $1 < \Reff < 40$ arcsec,
and stellar masses of $10^7 < \Mstar/\Msun < 10^{11.5}$. We find that:
(i)~the detection fraction of injected sources drops from ${\sim}95\%$ at
$\mur = 20$ to ${\sim}50\%$ at $\mur = 25$ and ${\sim}10\%$ at $\mur = 27$
$\mathrm{mag\,arcsec^{-2}}$; (ii)~for `realistic' sources (where we modify
size and colour along with surface brightness to produce realistic objects),
the recovered $r$-band flux is overestimated by ${\sim}100\%$ (a factor of
2) at $\mur = 27$, and by approximately an order of magnitude at $\mur =
30$; (iii)~in a controlled experiment at fixed $\Reff = 5$ arcsec, an
opposite trend emerges: flux is increasingly underestimated at fainter
surface brightnesses; (iv)~the 2D $\mur$ vs.\ \Reff analysis reveals that
the underestimation of flux is driven by extendedness, with sources at $\Reff
> 15$ arcsec showing systematic underestimates of $40$--$100\%$ over a wide
range of surface brightnesses; (v)~the injection of a bright source ($\mur =
18$) causes ${\sim}60\%$ of pre-injection nearest-neighbour detections to be
lost; (vi)~the resulting biases distort the galaxy stellar mass function
(GSMF) at all masses below $\log(\Mstar/\Msun) \sim 9$. These biases have
implications not only for LSB science, including dwarf galaxy demographics
\citep[e.g.][]{Benson2003}, the faint-end slope of the GSMF
\citep[e.g.][]{Baldry2012}, and intracluster light measurements
\citep[e.g.][]{Martin2019}, but also for weak lensing shear calibration
\citep[e.g.][]{Mandelbaum2018}, photometric redshift estimation, and
transient host-galaxy association. We outline next steps for mitigating
these biases within the Science Pipelines.
}
...
The LSST \citep{Ivezic2019} will image approximately 18\,000~deg$^2$ of
...
At the depth of the LSST co-adds, the surface density of detected sources is
sufficiently high that the majority of objects are blended with at least one
neighbour. Deblending --- the process of disentangling the flux contributions
of overlapping sources --- is therefore not a corner-case correction but a
fundamental step in catalogue construction. Errors in deblending propagate
directly into photometric measurements, and from there into derived quantities
including photometric redshifts \citep[e.g.][]{Benitez2000}, stellar masses
\citep[e.g.][]{Taylor2011,Bryant2015}, and shape measurements used for weak
lensing shear estimation \citep[e.g.][]{SheldonHuff2017}. For strong lensing
analyses, inaccurate deblending of arcs and multiply-imaged sources can
compromise lens modelling and time-delay cosmography
\citep[e.g.][]{Suyu2017}. In transient science, mis-association of a
supernova or other transient with the wrong host galaxy, due to deblending
errors, propagates into incorrect host-galaxy properties and potentially
biased cosmological inferences \citep[e.g.][]{Gupta2016,Sako2018}.

The importance of deblending is especially acute for low surface brightness
(LSB) science. The population of ultra-diffuse galaxies
\citep[UDGs;][]{vanDokkum2015}, classical LSB galaxies
\citep[e.g.][]{ImpeyBothun1997}, and dwarf galaxies
\citep[e.g.][]{Benson2003,Martin2019} is predicted to vastly outnumber the
high surface brightness galaxy population, yet they are precisely the objects
most vulnerable to being missed by detection algorithms or having their flux
misattributed by deblenders. Accurate photometry of these systems is
necessary to constrain the faint-end slope of the luminosity and mass
functions, which in turn constrain models of galaxy formation, feedback
processes, and the nature of dark matter
\citep[e.g.][]{BullockBoylanKolchin2017}. Similarly, measurements of
intracluster light (ICL), the diffuse stellar component of galaxy clusters
\citep[e.g.][]{Montes2019}, require the deblender to correctly apportion
flux between cluster member galaxies and the diffuse ICL component. Failures
in this apportionment directly bias ICL fraction estimates and the inferred
...
to measure detection completeness and photometric bias as a function of
surface brightness, effective radius, stellar mass, and local environment.
...
catalogue generation, and explain how our injections enter at the
single-exposure level. In Section~\ref{sec:scarlet} we give a technical
overview of the \scarletlite deblender. In Section~\ref{sec:failures} we
describe qualitative failure modes identified in real data. In
...
Section~\ref{sec:nextsteps} we outline next steps.
...
(Section~\ref{sec:methodology}), we inject synthetic sources into calibrated
...
\subsection{Deblending and Measurement}
...
multiband deblending using the \scarletlite algorithm described in
Section~\ref{sec:scarlet}. Each Footprint is treated as an independent blend
scene: the deblender fits a model to every Peak (source) within the Footprint
simultaneously, apportioning flux among the constituent sources. The deblended
source models are then used as templates for photometric measurement by
\task{MeasureMergedCoaddSourcesTask}, which measures fluxes, shapes, and other
...
Because our injections enter the pipeline at the single-exposure stage, they
undergo identical processing to real sources, including PSF convolution during
warping, noise-correlation corrections, and co-addition weighting. This
ensures that the detection, deblending, and measurement stages operate on data
that are statistically indistinguishable from a genuine observation containing
the injected source. By comparing the output catalogues from the modified and
unmodified coadds, we isolate the pipeline's response to sources of known
properties in a fully controlled manner.


% =============================================================================
\section{The Scarlet-lite Deblender}
\label{sec:scarlet}
% =============================================================================

The LSST Science Pipelines employ \scarletlite as the default multiband
deblender for coadded images. \scarletlite is a streamlined reimplementation
...
\subsection{Constrained Matrix Factorisation}
...
\subsection{Optimisation: Proximal Gradient Descent}
...
\subsection{Initialisation and Convergence}
...
\subsection{PSF Handling and Photometry}
...
\subsection{Key Limitation: Agnosticism to Detection Legitimacy}
\label{sec:agnostic}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image2}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image3}
...
To quantify the severity of the failure modes identified above, we designed a
suite of controlled injection experiments. We inject synthetic galaxy images
with known ground-truth photometric properties into calibrated single-exposure
images, process these modified exposures through the full LSST pipeline (from
warping through coaddition, detection, deblending, and measurement), and
compare the recovered catalogue properties against the input values and against
the catalogues produced from the unmodified coadds.
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{figures/image4}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image9}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image10}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image11}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image12}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/image13}
...
\begin{figure}
  \centering
  \includegraphics[width=0.85\textwidth]{figures/image14}
...
relies on accurate galaxy shape measurements, which depend on unbiased flux and
morphology models. The flux redistribution we observe between injected sources
and their neighbours suggests that the effective PSF model used for shape
measurement may be contaminated. The LSST DESC requirements
\citep{LSST_DESC2018} place stringent limits on multiplicative shear bias
($|m| < 0.003$), and it is important to determine whether deblending
systematics contribute at this level.
...
(optionally) symmetry. Introducing a data-driven morphological prior, learned
from e.g.\ HST or Euclid imaging, could regularise the deblender towards
physically plausible galaxy shapes. Recent work on score-matching neural
networks for galaxy morphology priors \citep{Sampson2024} offers a promising
...
detection thresholds, or a multi-resolution detection scheme that identifies
extended sources at lower spatial frequencies before detecting point sources,
could reduce the rate of spurious detections. An iterative feedback loop
between the deblender and the detector, in which \scarlet flags Peaks with
...
The monotonicity constraint is responsible for the left-behind flux failure
mode in sources with star-forming regions and spiral arms. A conditional
...
Our current analysis identifies the injected source by matching the nearest
detection to the injection position. If the source has been shredded, some of
its flux resides in spurious child sources. A post-hoc flux reassembly step,
which sums the flux of all child sources within the injection footprint, would
quantify whether the flux is being redistributed (recoverable) or lost entirely.
...
quantifying and addressing these biases as early as possible, and we look
forward to continued collaboration with the Data Management team and the
broader Rubin community.
